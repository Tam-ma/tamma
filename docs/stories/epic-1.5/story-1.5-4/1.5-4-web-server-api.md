...
11. GitHub App installation callback endpoint (`GET /api/github/callback`) handles post-install redirect: receives `installation_id` and `setup_action` query parameters, fetches installation details from GitHub API, persists to `IInstallationStore`, redirects user to success page
...
│   │   │   │   ├── config.ts
│   │   │   │   ├── metrics.ts
│   │   │   │   ├── github-callback.ts  # GitHub App installation callback (AC 11)
│   │   │   │   └── index.ts
