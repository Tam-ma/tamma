...
10. GitHub App `installation` webhook events handled: `created`, `deleted`, `suspended`, `unsuspended` — updates `IInstallationStore` accordingly
11. GitHub App `installation_repositories` webhook events handled: `added`, `removed` — updates repository associations in installation store
12. Installation lifecycle events tagged with `installation_id` for DCB audit trail
13. Installation webhook handler validates `installation.id` exists and is known before processing lifecycle events
...
### GitHub App Installation Event Handler (AC: 10, 11, 12, 13)

The following additions extend the webhook handler to support GitHub App installation lifecycle events. These events are critical for the SaaS deployment model (Stories 1.5-11 through 1.5-14).

**Task 8: GitHub App Installation Event Handler** (AC: 10, 12, 13)

- [ ] Subtask 8.1: Add `installation` event handling to `GitHubWebhookHandler.processEvent()` switch statement
- [ ] Subtask 8.2: Handle `installation.created` — call `IInstallationStore.save()` with installation details from payload
- [ ] Subtask 8.3: Handle `installation.deleted` — call `IInstallationStore.remove()` with `installation.id`
- [ ] Subtask 8.4: Handle `installation.suspended` — call `IInstallationStore.markSuspended()` with `installation.id`
- [ ] Subtask 8.5: Handle `installation.unsuspended` — call `IInstallationStore.clearSuspension()` with `installation.id`
- [ ] Subtask 8.6: Emit DCB events for each lifecycle transition: `INSTALLATION.CREATED`, `INSTALLATION.DELETED`, `INSTALLATION.SUSPENDED`, `INSTALLATION.UNSUSPENDED`
- [ ] Subtask 8.7: Validate `installation.id` exists in the payload before processing (return 400 if missing)

**Task 9: Installation Repository Events** (AC: 11)

- [ ] Subtask 9.1: Add `installation_repositories` event handling to `GitHubWebhookHandler.processEvent()`
- [ ] Subtask 9.2: Handle `installation_repositories.added` — update installation's repository list in store
- [ ] Subtask 9.3: Handle `installation_repositories.removed` — remove repositories from installation's list in store
- [ ] Subtask 9.4: Log repository changes with installation context for traceability

**Task 10: Tests for Installation Events** (AC: 10, 11, 12, 13)

- [ ] Subtask 10.1: Unit tests for `installation.created` event handling (persist to store, emit event)
- [ ] Subtask 10.2: Unit tests for `installation.deleted` event handling (remove from store, emit event)
- [ ] Subtask 10.3: Unit tests for `installation.suspended` / `unsuspended` event handling
- [ ] Subtask 10.4: Unit tests for `installation_repositories.added` / `removed` events
- [ ] Subtask 10.5: Unit tests for missing `installation.id` validation (400 response)
- [ ] Subtask 10.6: Integration tests for installation lifecycle flow: created → suspended → unsuspended → deleted

**Dependencies:** Story 1.5-11 provides `IInstallationStore` interface and implementations. This task can be implemented after Story 1.5-11 or concurrently if the interface is agreed upon upfront.
